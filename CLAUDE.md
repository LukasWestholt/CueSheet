...
At **runtime** the routine list resolves in priority order: a localStorage override (`tjf.tracks`, `src/data/tracksStore.ts`) wins; otherwise the **public-folder default set** (manifest entries flagged `"default": true` or named `default*.json` — here `playbook-2026.json` — loaded async on startup via `src/data/recommendedImports.ts#loadDefaultRoutines`) replaces the code-defined `TRACKS`; otherwise (no server / offline before precache) the code `DEFAULT_TRACKS` are the fallback. `App` seeds state synchronously from the store/code, then swaps in the public defaults once fetched (when there's no override). The **Routines panel** (`RoutinesManager`) imports/exports the list as JSON, lists every `public/routines.json` file (defaults show a "loaded" badge; non-defaults are one-tap import targets), has a **Contribute a routine** modal explaining the PR workflow (add a file to `public/` + an entry to `routines.json`), and **Reset** reloads the public defaults. `validateTracks` (`src/data/validateTracks.ts`) gates imports (dup ids = error, dup URIs = warning, plus a routine-length warning); `collectStepLibrary` (`src/data/stepLibrary.ts`) derives the distinct-move palette for the step editor.
...
- [x] **Import/export routines as JSON.** Back up and share the routine list without git; download/upload a file. (`RoutinesManager` + `validateTracks`.)
...
- [x] **Public-folder routine files.** Drop routine JSON in the web server's public folder; entries flagged `"default": true` (or named `default.json`/`default.*.json`) are **auto-loaded on startup** as the base set (replacing the code-defined `TRACKS` when present), and any other manifest-listed files appear as one-tap **import targets** in `RoutinesManager`. The committed default is `public/playbook-2026.json`. (`src/data/recommendedImports.ts` + `public/routines.json` manifest; precached for offline via the PWA `globPatterns`. `tracks.ts`'s `DEFAULT_TRACKS` is kept only as the no-server/offline fallback — the `Track` *type* still lives there.)
